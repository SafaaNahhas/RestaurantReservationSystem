            ...
            <table cellpadding="0" cellspacing="0" width="800" style="background-color: #ffffff; border-radius: 8px; overflow: hidden;">
                ...
                <tr>
                    <td bgcolor="#50C878" style="padding: 30px; text-align: center;">
                        <h1 style="color: #ffffff; margin: 0;">Daily Reservations Report</h1>
                    </td>
                </tr>
                    ...
                    <td style="padding: 30px;">
                        ...
                        <table width="100%" cellpadding="10" cellspacing="0" style="border-collapse: collapse; margin-top: 10px;">
                            ...
                            <tr style="background-color: #f7fafc;">
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">ID</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">Table ID</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">Guest Count</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">Status</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">Start Date</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">End Date</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">Created At</th>
                                <th style="border: 1px solid #e2e8f0; color: #4a5568; text-align: left;">Manager</th>
                            </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="border: 1px solid #e2e8f0;">
                                        @switch($reservation->status)
                                            @case('pending')
                                                <span style="color: #FFA500;">{{ $reservation->status }}</span>
                                                @break
                                            @case('confirmed')
                                                <span style="color: #2B7A0B;">{{ $reservation->status }}</span>
                                                @break
                                            @case('in_service')
                                                <span style="color: #3B82F6;">{{ $reservation->status }}</span>
                                                @break
                                            @case('completed')
                                                <span style="color: #059669;">{{ $reservation->status }}</span>
                                                @break
                                            @case('cancelled')
                                                <span style="color: #B31312;">{{ $reservation->status }}</span>
                                                @break
                                            @case('rejected')
                                                <span style="color: #7A1212;">{{ $reservation->status }}</span>
                                                @break
                                        @endswitch
                                    </td>
                                    ...
                                    <td style="border: 1px solid #e2e8f0; color: #2d3748;">{{ $reservation->manager ? $reservation->manager->name : 'N/A' }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
